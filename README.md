...
## If you would like run this demo, please download the two notebook files before running them in your workspace:

**1. You may directly view the code from the notebooks inside of the `notebooks` folder on Github** 


**2. Or run them on Databricks Community Edition (free).**
Note that Community Edition is different from the Databricks Free Trial version.
...
* Download the .ipynb notebooks inside of the `notebooks` folder on this repo, and import them into your Databricks enterprise user account.

* Run the notebooks on a cluster with Databricks runtime version `10.4 LTS ML`. Note: use `10.4 LTS ML`, not `10.4 LTS`.
