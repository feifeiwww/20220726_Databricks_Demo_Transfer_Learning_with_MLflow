...
step c) Create a cluster under the "compute" tab on the left, select Databricks runtime version 10.4ML or up. (note: use 10.4ML, not 10.4). Refer to this [doc](https://docs.databricks.com/clusters/create.html) about how to create a cluster. It may take a few minutes to start a cluster. 

step d) Open the imported notebooks on Databricks from the workspace tab, open either notebook part1, attach the cluster to your notebook, then click "Run All" on the top to run the notebook. You may refer to this [doc](https://docs.databricks.com/notebooks/notebooks-manage.html#attach-a-notebook-to-a-cluster) under the section "Attach a notebook to a cluster". 

step e) You can now try to open notebook part2 now, attach it to the 10.4ML cluster, and click "Run All".

step f) Important note: you must attach your notebook to a cluster before you can run it. If your cluster is terminated, you need to restart it or re-create a new cluster. 
